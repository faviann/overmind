...
for _ in $(seq 1 60); do
  if docker exec "$postgres" pg_isready -U overmind -d postgres >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
docker exec "$postgres" pg_isready -U overmind -d postgres >/dev/null
...
docker run --rm --network "$network" \
  ...
  --entrypoint memctl \
  "$image" migrate | grep -Fx 'migrations applied' >/dev/null
...
for _ in $(seq 1 60); do
  if curl --fail --silent --show-error "$health" >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
