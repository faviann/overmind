...
> 8. Test-DB location reconciled (see §2/§12): dev/test/CI use a disposable local `memory_test`; the LXC hosts production `memory` only. Language bullet aligned to the committed .NET stack.
...
Where this spec is silent and `evidence-and-knowledge-boundary.md` does not decide ownership, nothing else decides for it: raise the gap with the operator rather than inferring authority from an older document.
...
- **Source-of-truth hierarchy:** when two records disagree, authority runs **the current authoritative source (Git/IaC and equivalents) > approved memory > proposed memory > raw trace inference**. This decides which record wins; it mandates no mechanism. When an authoritative source changes, the existing requirement is only that dependents remain *identifiable* — the `source_id` index of acceptance test 2. Automated re-validation is the Phase 3 reconciliation worker (§13), not a Phase 1 obligation.
...
- **Never-store gate (v1.1 — split behavior; v1.5 deterministic detector, issue #76):** every write path runs a bounded deterministic scan before insert. The rule set in `config/never_store.yaml` is validated, versioned, uniquely identified, literal-prefiltered, and compiled once with `RegexOptions.NonBacktracking` under a per-rule timeout; missing, empty, invalid, duplicated, unsupported, or un-loadable configuration fails closed rather than loading zero rules. It covers private-key blocks, authentication headers, credential-bearing URLs, structured sensitive fields, a small versioned provider set, and operator-provisioned exact values supplied outside the tracked file. Matching operates on decoded structured leaf values — serialized JSON is never regex-rewritten — with at most one bounded percent/hex/Base64 decoding level around the high-confidence rules and deterministic overlap resolution. Numeric scan budgets are versioned runtime constants; exceeding one fails closed. See [capture safety budgets](capture-safety-budgets.md). Behavior differs by store:
  - **Memory writes (`propose_memory`, `save_note`): REJECT.** Return an error naming the rule — the highest-priority match the scan accepted, not whichever rule id sorts first; log a redacted `note` trace event that a write was blocked.
  - **Trace writes (`log_trace` and server-side auto-logging): REDACT in place.** Replace each matched span with `[REDACTED:<rule-name>]` and record the event. Structured property *names* cross the same rules as values, so a credential used as a map key is redacted too. Where no exact span can be mapped — a leaf beyond the per-leaf byte limit, a governed sensitive field carrying a subtree or a non-string scalar, or two sibling property names that collapse to one key after redaction — the whole leaf or object is replaced with the distinct marker `[OMITTED:<reason>]` and its safe siblings are retained. A tool result that happens to contain a token still describes something that happened; dropping the event silently would trade a secrets risk for an audit hole, and the acceptance tests are trace joins. Redaction happens *before* insert — it is write-time sanitization, not mutation of an append-only row.
...
**Wire convention:** JSON tool-response property names use camelCase, following
the MCP SDK convention. Snake_case names elsewhere in this document are
conceptual or storage-layer names; PostgreSQL columns such as `trace_uuid` are
genuinely snake_case and remain so.

**Core (must ship):**

1. **`log_trace`** `{event_type, content, refs?, namespace?}` → `{traceUuid, sessionId}`
   `next`: "If this event contains a durable decision or fact, call propose_memory referencing this trace_uuid as source_id."
   **(v1.3)** The session is server-derived, never a tool argument; a caller-supplied `session_id` is ignored and the effective session is echoed in the response.
2. **`search_memory`** `{query, namespaces?, types?, limit?}` → `{results: [{uuid, type, tier, status, preview, sourceType, sourceId, version, laneScores, fusedScore}], next}`
...
3. **`get_by_id`** `{uuid}` → full record incl. all provenance columns + version chain (`supersedes` / superseded-by).
   `next` when `source_id` is present: "This memory derives from source_id=<...>; retrieve_trace on it for full context." When `source_id` is null: "This memory has no recorded source; call search_memory for related context, or log_trace to capture supporting evidence." (Server logs `memory_consumed`.)
...
- ❌ Auto-injection of memory into an agent's context — memory is a tool surface the agent calls, never middleware that predicts what the agent needs and prepends it. The agent asks, the trace shows what it asked for, and the cost follows its own choices
